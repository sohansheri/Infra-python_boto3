...
data "aws_iam_policy_document" "lab_lambda_for_s3_access" {
  statement {
    ...
    resources = [
      "arn:aws:s3:::${aws_s3_bucket.s3bucket_detect_faces.bucket}",
      "arn:aws:s3:::${aws_s3_bucket.s3bucket_detect_faces.bucket}/*"
    ]
  }
}
...
resource "aws_lambda_permission" "allow_bucket" {
  ...
  function_name = aws_lambda_function.lambdafunction_image_rekognition.arn
  principal     = "s3.amazonaws.com"
  source_arn    = aws_s3_bucket.s3bucket_detect_faces.arn
}